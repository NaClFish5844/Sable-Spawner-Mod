...
import dev.sablespawner.manager.datapack.property.sublevel.PrefabProperty;
...
public final class DatapackChecker {
    ...
    public static FileType interpretFileType(JsonObject object) {
        ...
        if ( isProperty == 0 && isWorldConfig == 0 ) {
            getLogger().debug("推断失败 | Failed to infer file type");
            return FileType.invalid;
        }
        if ( isProperty >= isWorldConfig ) {
            getLogger().debug("推断为蓝图属性文件 | Inferred as property file");
            return FileType.property;
        }
        else {
            getLogger().debug("推断为维度配置文件 | Inferred as worldconfig file");
            return FileType.worldconfig;
        }
    }
    public static boolean checkDefaultWorldConfigFormat(JsonObject object) {
        getLogger().debug("正在检查默认值配置文件 | Checking default config");
    ...
    public static WorldConfigCheckResult checkWorldConfigFormat(JsonObject object) {
        getLogger().debug("正在检查维度配置文件格式 | Checking worldconfig format");
    ...
    public static PropertyCheckResult checkPropertyFormat(JsonObject object) {
        getLogger().debug("正在检查蓝图属性文件格式 | Checking property format");
        ...
        if ( !isTypeKeysValid ) {
            getLogger().warn("[sublevel_types] 无效，将不生成具体属性 | [sublevel_types] invalid, no property will be generated");
    ...
    public static boolean checkPropertyBlock( String key, JsonElement element, Class<? extends AbstractSchematicProperty> propertyClass) {
        ...
        for (String field : element.getAsJsonObject().keySet()) {
            if (!propertyKeys.contains(field)) {
                getLogger().warn("[{}] {} 为未知字段，将被忽略 | [{}] {} is an unknown field, ignored", key, field, key, field);
            }
        }
    ...
    public static boolean checkNumberInRange( String key, JsonObject root, Number min, Number max ) {
        ...
        if ( Double.isNaN(value) ) {
            getLogger().warn("[{}] 值为 NaN | [{}] value is NaN", key, key);
            return false;
        }
        if ( value < lowerBound || value > upperBound ) {
            getLogger().warn("[{}] 超出范围 | [{}] out of range", key, key);
            return false;
        }
        return true;
    }
    public static boolean checkStringInEnum( String key, JsonObject root, Class< ? extends Enum<?> > enumClass ) {
        ...
        if ( !valid.contains(value) ) {
            getLogger().warn("[{}] 名称非法，为{}，（合法值为{}） | [{}] invalid name: {}, (valid: {})", key, value, validValues, key, value, validValues);
            return false;
        }
    ...
    public static boolean checkStringsInEnum( String key, JsonObject root, Class< ? extends Enum<?> > enumClass ) {
        ...
        for ( String v : values ) {
            if ( !valid.contains(v) ) {
                getLogger().warn("[{}] 中发现非法值，为{}，（合法值为{}） | [{}] contains invalid value: {}, (valid: {})", key, v, validValues, key, v, validValues);
                return false;
            }
        }
    ...
    public static boolean checkNumberArrListSorted( String key, JsonObject root ) {
        ...
        for (int i = 1; i < array.size(); i++) {
            ...
            if ( c < p ) {
                getLogger().warn( "[{}] 发现列表未按升序排列 | [{}] list is not in ascending order", key, key );
                return false; }
            if ( c == p ) {
                getLogger().warn( "[{}] 发现列表存在重复值 | [{}] list contains duplicate values", key, key );
                return false;
            }
        }
        return true;
    ...
    public static boolean isNumber( String key, JsonElement element ) {
        if (
            ...
            getLogger().warn("[{}] 缺失或不是数字 | [{}] missing or not a number", key, key);
            return false;
        }
        return true;
    }
    public static boolean isInteger( String key, JsonElement element ) {
        ...
        if ( element.getAsJsonPrimitive().getAsBigDecimal().stripTrailingZeros().scale() > 0 ) {
            getLogger().warn("[{}] 不是整数 | [{}] not an integer", key, key);
        }
        return true;
    }
    public static boolean isString( String key, JsonElement element ) {
        if (
            ...
            getLogger().warn("[{}] 缺失或不是字符串 | [{}] missing or not a string", key, key);
            return false;
        }
        return true;
    }
    public static boolean isArrList( String key, JsonElement element ) {
        if (
            ...
            getLogger().warn("[{}] 缺失或不是列表 | [{}] missing or not a list", key, key);
            return false;
        }
        return true;
    }
    public static boolean isBoolean( String key, JsonElement element ) {
        if (
            ...
            getLogger().warn("[{}] 缺失或不是布尔值 | [{}] missing or not a boolean", key, key);
            return false;
        }
        return true;
    }
