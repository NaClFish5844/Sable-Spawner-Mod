...
public class BlueprintManager {
    ...
    public void reloadBlueprint() {
        ...
        if ( BLUEPRINT_REGISTRY.isEmpty() ) {
            getLogger().info("未扫描到任何蓝图 | No blueprint scanned");
            return;
        } else {
            getLogger().info("扫描到 {} 个蓝图 | Scanned {} blueprints", BLUEPRINT_REGISTRY.size(), BLUEPRINT_REGISTRY.size());
        }

        PROPERTY_BLUEPRINT_MAP.putAll(BlueprintLoader.loadPropertyBlueprintMap());
        if ( PROPERTY_BLUEPRINT_MAP.isEmpty() ) {
            getLogger().info("未发现任何蓝图属性与蓝图的映射 | No property-blueprint mapping found");
            return;
        } else {
            getLogger().info("发现 {} 个蓝图映射 | Found {} blueprint mappings", PROPERTY_BLUEPRINT_MAP.size(), PROPERTY_BLUEPRINT_MAP.size());
        }

        Object2ObjectOpenHashMap<BlueprintKey, BlueprintEntry> bufferSlice = BlueprintLoader.loadBuffer();
        if ( bufferSlice.isEmpty() ) {
            getLogger().info("未向缓存载入蓝图 | No blueprint loaded into cache");
        } else {
            getLogger().info("向内存中缓存了 {} 个蓝图 | Cached {} blueprints", bufferSlice.size(), bufferSlice.size());
